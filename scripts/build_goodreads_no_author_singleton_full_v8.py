...
OUT = ROOT / "derived/goodreads_no_author_singleton_full_v8.tsv"
OUT_TRIAGE = (
    ROOT
    / "derived/goodreads_no_author_singleton_full_v8_temporal_triage.tsv"
)
...
target = v8[
    v8["goodreads_resolution_status"].eq("NO_AUTHOR_SUPPORT")
    & v8["goodreads_candidate_count"].eq("1")
].copy()
...
assert len(x) == 932
assert (x["source_batch"] == "old_1122").sum() == 916
