...
# ──────────────────────────────────────────────
_REF_PATTERNS = [
    # ("PGS," p. 170) / ("I," pp. 5, 6) — クォート略語・複数ページ対応（広めパターン）
    re.compile(r'\(["\u201c]?[A-Z]{1,6}[,\u201d"]*\s*pp?\.\s*[\d,\s]+\)'),
    # ("CC,"p.603) — スペースなしページ参照
    re.compile(r'\(["\u201c][A-Z]{1,6}[,.]?["\u201d]\s*p\.\s*\d{1,4}\)'),
    # (PL,p.193) / (PL, p.193) — クォートなし略語
    re.compile(r'\([A-Z]{1,6},\s*pp?\.\s*\d{1,4}(?:[,\s]\d{1,4})?\)'),
    # [S,p.50] / [S, p.50] — 角括弧略語（Gavin方式）
    re.compile(r'\[[A-Z]{1,6},\s*pp?\.\s*\d{1,4}(?:[,\s]\d{1,4})?\]'),
    # ["W," p. 49] — 角括弧クォート略語（Gavin方式）
    re.compile(r'\[["\u201c][A-Z]{1,6}[,.]?["\u201d]\s*pp?\.\s*\d{1,4}(?:[,\s]\d{1,4})?\]'),
    # (Foucault 1977, 23) / (Foucault 1977) — 著者年形式
        ...
        with pdfplumber.open(str(pdf_path)) as pdf:
            ...
            for page_idx, page in enumerate(pdf.pages):
                ...
                def flush_block():
                    ...
                    # blockは参照がtext末尾に含まれることが多い（例: [S,p.50]）
                    # text末尾50文字 + context_after を結合して検索
                    ref = extract_ref_hint(text[-50:] + " " + ctx_a)
