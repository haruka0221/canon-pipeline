...
OUT = Path("derived/goodreads_base_review_v3.tsv")
...
# PRIMARY Goodreads contributors
...
for ol_key, rows in supported.items():
    ...
    # unique author-supported candidate only
    if len(rows) != 1:
        continue

    r = rows[0]

    if r["title_match_type"] not in {
        "WORK_BASE",
        "BOOK_BASE",
    }:
        continue
...
print("REVIEW_BASE rows:", len(review))
print("output:", OUT)
