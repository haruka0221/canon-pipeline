...
OUT = ROOT / "derived/goodreads_no_author_singleton_full_v9.tsv"
OUT_TRIAGE = (
    ROOT
    / "derived/goodreads_no_author_singleton_full_v9_temporal_triage.tsv"
)

v9 = pd.read_csv(V9, sep="\t", dtype=str).fillna("")
...
target = v9[
    v9["goodreads_resolution_status"].eq("NO_AUTHOR_SUPPORT")
    & v9["goodreads_candidate_count"].eq("1")
].copy()
...
assert len(x) == 925
assert (x["source_batch"] == "old_1122").sum() == 909
