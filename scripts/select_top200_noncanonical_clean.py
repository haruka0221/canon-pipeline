...
# 除外リスト（canonical作品の重複work_key・非英語）
exclude_work_keys = [
    "/works/OL468431W",   # The Great Gatsby（重複）
    "/works/OL19908W",    # Kim（重複）
    "/works/OL498556W",   # Die Verwandlung（非英語）
]
nc = nc[~nc["work_key"].isin(exclude_work_keys)]

# さらに非ASCII文字を含むタイトルを除外
import unicodedata
def has_nonascii(s):
    if pd.isna(s): return False
    return any(ord(c) > 127 for c in str(s))
nc = nc[~nc["title"].apply(has_nonascii)]
