...
RESULTS = PILOT / "results_jev_native_v2_jev-1.13.0.jsonl"
FAILURES = PILOT / "failures_jev_native_v2_jev-1.13.0.jsonl"
...
def load_jsonl_by_target(path, wanted):
    out = {}
...
for pos, sel in enumerate(selection, start=1):
    ...
    # Absolute candidate-by-candidate judgments.
    ...
    result = {
        "experiment": "jev_native_v2_pilot15_seed20260921",
        "target_id": tid,
        "target": {
            "title": packet["title"],
    ...
    print(
        "    Noul top:",
        ranked_noul[0]["qid"],
        f'p={ranked_noul[0]["same_work_probability"]:.3f}',
    )

    print(
        "    Choice:",
        choice_answer.choice,
        f'confidence={float(choice_answer.confidence):.3f}',
    )
    ...
    print(
        ...
        " ".join(
            f"{qid}={prob:.3f}"
            for qid, prob in ranked_choice[:3]
        ),
    )
